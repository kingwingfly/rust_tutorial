...
此图中，同一列的两个加号标记出一段生命周期，名为`'a 'b`，例如，第2行到第10行为`r`的生命周期，名为`'a`。
...
早期，Rust中生命周期是从创建到作用域结束，即`}`；后来，变为从创建到最后一次使用。详见：[NLL (Non-Lexical Lifetime)](https://rust-lang.github.io/rfcs/2094-nll.html)
...
A：闭包的特征。意思是，泛型`F`上有个`call_once`方法，至少能调用一次，没有传入参数，返回值类型为`T`。
